...
inst_trace_t::inst_trace_t(const inst_trace_t &b) {
  ...
  for (unsigned i = 0; i < reg_dsts_num; ++i) reg_dest[i] = b.reg_dest[i];
  opcode = b.opcode;
  reg_srcs_num = b.reg_srcs_num;
  for (unsigned i = 0; i < reg_srcs_num; ++i) reg_src[i] = b.reg_src[i];
...
kernel_trace_t *trace_parser::parse_kernel_info(
  ...
  if (kerneltraces_filepath.rfind(".gz") == std::string::npos) //trace is not .gz - JH
  {
    std::cout << "File is not compressed: " << kerneltraces_filepath << std::endl;
    exit(1);
  }
  kernel_info->ifs = new igzstream;
  igzstream *ifs = kernel_info->ifs;
...
void trace_parser::get_next_threadblock_traces(
    ...
    unsigned trace_version, igzstream *ifs) {
